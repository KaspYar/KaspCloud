...
// 498693264323-9j16ok7mctc2deuj2cfptgps97hek2q1.apps.googleusercontent.com
...
public class ConferenceApi {
    ...
    public Profile saveProfile(User user, ProfileForm profileForm) throws UnauthorizedException {
        ...
        if (profile == null) {
            ...
            if (displayName != null && teeShirtSize != null)
    ...
    public Profile getProfile(final User user) throws UnauthorizedException {
        ...
        Profile profile = (Profile) OfyService.ofy().load().key(key).now();
        return profile;
    }
    ...
    public Conference createConference(final User user, final ConferenceForm conferenceForm)
        ...
        if (profile == null) {
            profile = new Profile(userId, "as", "as@as.as", TeeShirtSize.L);
        }
     ...
     *
     * @param user An user who invokes this method, null when the user is nKey.create(websafeConferenceKey)ot signed in.
    ...
    public Collection<Conference> getConferencesToAttend(final User user)
        ...
        List<Key<Conference>> keyConferences = new ArrayList<>();
        for (String s : keyStringsToAttend) {
            keyConferences.add(Key.<Conference>create(s));
        }

        return ofy().load().keys(keyConferences).values();  // change this
    }
